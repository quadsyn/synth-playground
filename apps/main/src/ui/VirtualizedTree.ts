...
function filteredNode(node: TreeNode, filter: string): TreeNode | null {
    if (node.children == null) {
        ...
        for (let i: number = 0; i < node.children.length; i++) {
            ...
            if (filtered != null) newChildren.push(filtered);
        }
...
function filteredTree(roots: TreeNode[], filter: string): TreeNode[] {
    ...
    for (let i: number = 0; i < roots.length; i++) {
        ...
        if (filtered != null) newRoots.push(filtered);
    }
    return newRoots;
}
...
function rebuildTreeViewState(state: TreeViewState): void {
    ...
    if (state.filter != null) roots = filteredTree(roots, state.filter);
...
export class VirtualizedTree implements Component {
    ...
    public expandAll(): void {
        ...
        function inner(node: TreeNode): void {
            if (node.children != null) {
                ...
                for (let child of node.children) inner(child);
            }
        }
        if (state.expanded.size === 0) {
            for (let child of state.root.children!) inner(child);
        } else {
    ...
    public render(): void {
        ...
        for (let i: number = 0; i < this._renderedRows.length; i++) {
            const renderedRow: TreeViewItem = this._renderedRows[i];
    ...
    private _handleClick = (event: MouseEvent): void => {
        ...
        for (let i: number = 0; i < this._renderedRows.length; i++) {
            const y: number = (i - topRowIndexResidue) * rowHeight;
            const dataIndex: number = topRowIndex + i;
            if (
                mouseY >= y
                && mouseY <= y + rowHeight
                && dataIndex >= 0
