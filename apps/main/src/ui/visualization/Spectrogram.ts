...
export class Spectrogram implements ManualComponent {
    ...
    private _renderSpectrogram(): void {
        ...
        for (let y: number = 0; y < height; y++) {
            ...
            const amplitude: number = unlerp(clamp(lerp(binT, bin0, bin1), minDb, maxDb), minDb, maxDb);
            const paletteEntryIndex: number = clamp((amplitude * paletteSize) | 0, 0, paletteSize - 1);
